...
# 全局属性
# Klamist 魔改说明
k_text = '''Klamist自用非疾跑修改版。
不疾跑时容错率很高，但慢一些。
程序系统框架照搬：
- Souloco的StarRailFastrun
地图路线参考资料：
- Starry-Wind的StarRailAssistant
魔改版项目地址：
https://github.com/Klamist/StarRail-SlowRun'''
# 标题
...
if __name__ == '__main__':
    ...
    ttk.Label(announce_frame,text='原始项目地址:',font=versionfont).grid(column=0,row=2)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            announce_text.insert('end',k_text)
            if lowversion > VER:
                ver_update = False
